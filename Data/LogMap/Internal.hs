...
(!) :: Ord k => LogMap k a -> k -> a
m ! k    = find k m

-- | Same as 'difference'.
(\\) :: Ord k => LogMap k a -> LogMap k b -> LogMap k a
m1 \\ m2 = difference m1 m2

{--------------------------------------------------------------------
  Size balanced trees.
--------------------------------------------------------------------}
-- | A LogMap from keys @k@ to values @a@.
data LogMap k a = Tip
              | Bin {-# UNPACK #-} !Size !k a !(LogMap k a) !(LogMap k a)

type Size     = Int

instance (Ord k) => Monoid (LogMap k v) where
    mempty  = empty
    mappend = union
    mconcat = unions
...
instance (Data k, Data a, Ord k) => Data (LogMap k a) where
  gfoldl f z m   = z fromList `f` toList m
  toConstr _     = error "toConstr"
  gunfold _ _    = error "gunfold"
  dataTypeOf _   = mkNoRepType "Data.LogMap.LogMap"
...
--
-- > Data.LogMap.null (empty)           == True
-- > Data.LogMap.null (singleton 1 'a') == False

null :: LogMap k a -> Bool
null t
  = case t of
      Tip    -> True
      Bin {} -> False
...
lookup :: Ord k => k -> LogMap k a -> Maybe a
...
lookupAssoc :: Ord k => k -> LogMap k a -> Maybe (k,a)
lookupAssoc  k t
  = case t of
      Tip -> Nothing
      Bin _ kx x l r
...
member :: Ord k => k -> LogMap k a -> Bool
member k m
  = case lookup k m of
      Nothing -> False
      Just _  -> True
...
notMember :: Ord k => k -> LogMap k a -> Bool
notMember k m = not $ member k m

-- | /O(log n)/. Find the value at a key.
-- Calls 'error' when the element can not be found.
find :: Ord k => k -> LogMap k a -> a
find k m
  = case lookup k m of
      Nothing -> error "LogMap.find: element not in the map"
      Just x  -> x
...
findWithDefault :: Ord k => a -> k -> LogMap k a -> a
...
insert :: Ord k => k -> a -> LogMap k a -> LogMap k a
...
insertWith :: Ord k => (a -> a -> a) -> k -> a -> LogMap k a -> LogMap k a
insertWith f k x m
...
insertWithKey :: Ord k => (k -> a -> a -> a) -> k -> a -> LogMap k a -> LogMap k a
...
insertLookupWithKey :: Ord k => (k -> a -> a -> a) -> k -> a -> LogMap k a -> (Maybe a,LogMap k a)
...
delete :: Ord k => k -> LogMap k a -> LogMap k a
delete _ Tip                 = Tip
...
adjust :: Ord k => (a -> a) -> k -> LogMap k a -> LogMap k a
adjust f k m
...
adjustWithKey :: Ord k => (k -> a -> a) -> k -> LogMap k a -> LogMap k a
adjustWithKey f k m
...
update :: Ord k => (a -> Maybe a) -> k -> LogMap k a -> LogMap k a
update f k m
...
updateWithKey :: Ord k => (k -> a -> Maybe a) -> k -> LogMap k a -> LogMap k a
updateWithKey f k t
  = case t of
      Tip -> Tip
      Bin sx kx x l r
...
updateLookupWithKey :: Ord k => (k -> a -> Maybe a) -> k -> LogMap k a -> (Maybe a,LogMap k a)
...
-- 'alter' can be used to insert, delete, or update a value in a 'LogMap'.
...
alter :: Ord k => (Maybe a -> Maybe a) -> k -> LogMap k a -> LogMap k a
alter f k t
  = case t of
...
findIndex :: Ord k => k -> LogMap k a -> Int
findIndex k t
  = case lookupIndex k t of
      Nothing  -> error "LogMap.findIndex: element is not in the map"
      Just idx -> idx
...
lookupIndex :: Ord k => k -> LogMap k a -> Maybe Int
...
elemAt :: Int -> LogMap k a -> (k,a)
elemAt _ Tip = error "LogMap.elemAt: index out of range"
...
updateAt :: (k -> a -> Maybe a) -> Int -> LogMap k a -> LogMap k a
updateAt _ _ Tip  = error "LogMap.updateAt: index out of range"
...
deleteAt :: Int -> LogMap k a -> LogMap k a
deleteAt i m
...
findMin :: LogMap k a -> (k,a)
findMin (Bin _ kx x Tip _)  = (kx,x)
findMin (Bin _ _  _ l _)    = findMin l
findMin Tip                 = error "LogMap.findMin: empty map has no minimal element"
...
findMax :: LogMap k a -> (k,a)
findMax (Bin _ kx x _ Tip)  = (kx,x)
findMax (Bin _ _  _ _ r)    = findMax r
findMax Tip                 = error "LogMap.findMax: empty map has no maximal element"
...
deleteMin :: LogMap k a -> LogMap k a
...
deleteMax :: LogMap k a -> LogMap k a
...
updateMin :: (a -> Maybe a) -> LogMap k a -> LogMap k a
updateMin f m
...
updateMax :: (a -> Maybe a) -> LogMap k a -> LogMap k a
updateMax f m
...
updateMinWithKey :: (k -> a -> Maybe a) -> LogMap k a -> LogMap k a
...
updateMaxWithKey :: (k -> a -> Maybe a) -> LogMap k a -> LogMap k a
...
minViewWithKey :: LogMap k a -> Maybe ((k,a), LogMap k a)
...
maxViewWithKey :: LogMap k a -> Maybe ((k,a), LogMap k a)
...
minView :: LogMap k a -> Maybe (a, LogMap k a)
minView Tip = Nothing
...
maxView :: LogMap k a -> Maybe (a, LogMap k a)
maxView Tip = Nothing
...
unions :: Ord k => [LogMap k a] -> LogMap k a
unions ts
...
unionsWith :: Ord k => (a->a->a) -> [LogMap k a] -> LogMap k a
unionsWith f ts
...
union :: Ord k => LogMap k a -> LogMap k a -> LogMap k a
union Tip t2  = t2
union t1 Tip  = t1
...
hedgeUnionL :: Ord a
            => (a -> Ordering) -> (a -> Ordering) -> LogMap a b -> LogMap a b
            -> LogMap a b
...
unionWith :: Ord k => (a -> a -> a) -> LogMap k a -> LogMap k a -> LogMap k a
unionWith f m1 m2
...
unionWithKey :: Ord k => (k -> a -> a -> a) -> LogMap k a -> LogMap k a -> LogMap k a
...
difference :: Ord k => LogMap k a -> LogMap k b -> LogMap k a
difference Tip _   = Tip
difference t1 Tip  = t1
difference t1 t2   = hedgeDiff (const LT) (const GT) t1 t2

hedgeDiff :: Ord a
          => (a -> Ordering) -> (a -> Ordering) -> LogMap a b -> LogMap a c
          -> LogMap a b
hedgeDiff _     _     Tip _
  = Tip
...
differenceWith :: Ord k => (a -> b -> Maybe a) -> LogMap k a -> LogMap k b -> LogMap k a
...
differenceWithKey :: Ord k => (k -> a -> b -> Maybe a) -> LogMap k a -> LogMap k b -> LogMap k a
...
intersection :: Ord k => LogMap k a -> LogMap k b -> LogMap k a
intersection m1 m2
...
intersectionWith :: Ord k => (a -> b -> c) -> LogMap k a -> LogMap k b -> LogMap k c
...
--intersectionWithKey :: Ord k => (k -> a -> b -> c) -> LogMap k a -> LogMap k b -> LogMap k c
...
--
isSubmapOf :: (Ord k,Eq a) => LogMap k a -> LogMap k a -> Bool
isSubmapOf m1 m2
  = isSubmapOfBy (==) m1 m2
...
-}
isSubmapOfBy :: Ord k => (a->b->Bool) -> LogMap k a -> LogMap k b -> Bool
isSubmapOfBy f t1 t2
  = (size t1 <= size t2) && (submap' f t1 t2)

submap' :: Ord a => (b -> c -> Bool) -> LogMap a b -> LogMap a c -> Bool
submap' _ Tip _ = True
...
isProperSubmapOf :: (Ord k,Eq a) => LogMap k a -> LogMap k a -> Bool
...
-}
isProperSubmapOfBy :: Ord k => (a -> b -> Bool) -> LogMap k a -> LogMap k b -> Bool
...
filter :: Ord k => (a -> Bool) -> LogMap k a -> LogMap k a
filter p m
...
filterWithKey :: Ord k => (k -> a -> Bool) -> LogMap k a -> LogMap k a
...
partition :: Ord k => (a -> Bool) -> LogMap k a -> (LogMap k a,LogMap k a)
partition p m
...
partitionWithKey :: Ord k => (k -> a -> Bool) -> LogMap k a -> (LogMap k a,LogMap k a)
...
-- | /O(n)/. LogMap values and collect the 'Just' results.
--
-- > let f x = if x == "a" then Just "new a" else Nothing
-- > mapMaybe f (fromList [(5,"a"), (3,"b")]) == singleton 5 "new a"

mapMaybe :: Ord k => (a -> Maybe b) -> LogMap k a -> LogMap k b
mapMaybe f m
  = mapMaybeWithKey (\_ x -> f x) m

-- | /O(n)/. LogMap keys\/values and collect the 'Just' results.
--
-- > let f k _ = if k < 5 then Just ("key : " ++ (show k)) else Nothing
-- > mapMaybeWithKey f (fromList [(5,"a"), (3,"b")]) == singleton 3 "key : 3"

mapMaybeWithKey :: Ord k => (k -> a -> Maybe b) -> LogMap k a -> LogMap k b
mapMaybeWithKey _ Tip = Tip
mapMaybeWithKey f (Bin _ kx x l r) = case f kx x of
  Just y  -> join kx y (mapMaybeWithKey f l) (mapMaybeWithKey f r)
  Nothing -> merge (mapMaybeWithKey f l) (mapMaybeWithKey f r)

-- | /O(n)/. LogMap values and separate the 'Left' and 'Right' results.
--
...
mapEither :: Ord k => (a -> Either b c) -> LogMap k a -> (LogMap k b, LogMap k c)
mapEither f m
  = mapEitherWithKey (\_ x -> f x) m

-- | /O(n)/. LogMap keys\/values and separate the 'Left' and 'Right' results.
--
...
mapEitherWithKey :: Ord k =>
  (k -> a -> Either b c) -> LogMap k a -> (LogMap k b, LogMap k c)
...
{--------------------------------------------------------------------
  Mapping
--------------------------------------------------------------------}
-- | /O(n)/. LogMap a function over all values in the map.
--
-- > map (++ "x") (fromList [(5,"a"), (3,"b")]) == fromList [(3, "bx"), (5, "ax")]

map :: (a -> b) -> LogMap k a -> LogMap k b
map f m
...
mapWithKey :: (k -> a -> b) -> LogMap k a -> LogMap k b
...
mapAccum :: (a -> b -> (a,c)) -> a -> LogMap k b -> (a,LogMap k c)
mapAccum f a m
...
mapAccumWithKey :: (a -> k -> b -> (a,c)) -> a -> LogMap k b -> (a,LogMap k c)
mapAccumWithKey f a t
  = mapAccumL f a t

-- | /O(n)/. The function 'mapAccumL' threads an accumulating
-- argument throught the map in ascending order of keys.
mapAccumL :: (a -> k -> b -> (a,c)) -> a -> LogMap k b -> (a,LogMap k c)
mapAccumL f a t
  = case t of
      Tip -> (a,Tip)
      Bin sx kx x l r
...
mapAccumRWithKey :: (a -> k -> b -> (a,c)) -> a -> LogMap k b -> (a,LogMap k c)
...
mapKeys :: Ord k2 => (k1->k2) -> LogMap k1 a -> LogMap k2 a
...
mapKeysWith :: Ord k2 => (a -> a -> a) -> (k1->k2) -> LogMap k1 a -> LogMap k2 a
...
mapKeysMonotonic :: (k1->k2) -> LogMap k1 a -> LogMap k2 a
...
-- > foldWithKey f "LogMap: " (fromList [(5,"a"), (3,"b")]) == "LogMap: (5:a)(3:b)"
--
...
foldWithKey :: (k -> a -> b -> b) -> b -> LogMap k a -> b
foldWithKey f z t
  = foldrWithKey f z t
...
-- | /O(n)/. In-order fold.
foldi :: (k -> a -> b -> b -> b) -> b -> LogMap k a -> b
foldi _ z Tip               = z
...
foldrWithKey :: (k -> a -> b -> b) -> b -> LogMap k a -> b
...
-- value to the lowest.
foldlWithKey :: (b -> k -> a -> b) -> b -> LogMap k a -> b
...
fromListWith :: Ord k => (a -> a -> a) -> [(k,a)] -> LogMap k a
fromListWith f xs
...
fromListWithKey :: Ord k => (k -> a -> a -> a) -> [(k,a)] -> LogMap k a
...
toAscList :: LogMap k a -> [(k,a)]
toAscList t   = foldrWithKey (\k x xs -> (k,x):xs) [] t

-- | /O(n)/. Convert to a descending list.
toDescList :: LogMap k a -> [(k,a)]
...
fromAscList :: Eq k => [(k,a)] -> LogMap k a
fromAscList xs
...
fromAscListWith :: Eq k => (a -> a -> a) -> [(k,a)] -> LogMap k a
...
fromAscListWithKey :: Eq k => (k -> a -> a -> a) -> [(k,a)] -> LogMap k a
...
fromDistinctAscList :: [(k,a)] -> LogMap k a
...
--------------------------------------------------------------------}
trim :: (k -> Ordering) -> (k -> Ordering) -> LogMap k a -> LogMap k a
trim _     _     Tip = Tip
...
trimLookupLo :: Ord k => k -> (k -> Ordering) -> LogMap k a -> (Maybe (k,a), LogMap k a)
...
--------------------------------------------------------------------}
filterGt :: Ord k => (k -> Ordering) -> LogMap k a -> LogMap k a
filterGt _   Tip = Tip
...
filterLt :: Ord k => (k -> Ordering) -> LogMap k a -> LogMap k a
filterLt _   Tip = Tip
...
split :: Ord k => k -> LogMap k a -> (LogMap k a,LogMap k a)
split _ Tip = (Tip,Tip)
...
splitLookup :: Ord k => k -> LogMap k a -> (LogMap k a,Maybe a,LogMap k a)
...
-- | /O(log n)/.
splitLookupWithKey :: Ord k => k -> LogMap k a -> (LogMap k a,Maybe (k,a),LogMap k a)
...
{--------------------------------------------------------------------
  Join
--------------------------------------------------------------------}
join :: Ord k => k -> a -> LogMap k a -> LogMap k a -> LogMap k a
...
--------------------------------------------------------------------}
merge :: LogMap k a -> LogMap k a -> LogMap k a
merge Tip r   = r
merge l Tip   = l
...
--------------------------------------------------------------------}
glue :: LogMap k a -> LogMap k a -> LogMap k a
glue Tip r = r
glue l Tip = l
glue l r
...
deleteFindMin :: LogMap k a -> ((k,a),LogMap k a)
...
deleteFindMax :: LogMap k a -> ((k,a),LogMap k a)
...
balanceL :: k -> a -> LogMap k a -> LogMap k a -> LogMap k a
balanceL k x l r
  | isBalanced l r = bin k x l r
  | otherwise      = rotateL k x l r

balanceR :: k -> a -> LogMap k a -> LogMap k a -> LogMap k a
balanceR k x l r
...
----------------------------------------------------------------
-- basic rotations
singleL :: a -> b -> LogMap a b -> LogMap a b -> LogMap a b
singleL k1 x1 t1 (Bin _ k2 x2 t2 t3)  = bin k2 x2 (bin k1 x1 t1 t2) t3
singleL _ _ _ _ = error "singleL"

singleR :: a -> b -> LogMap a b -> LogMap a b -> LogMap a b
singleR k1 x1 (Bin _ k2 x2 t1 t2) t3  = bin k2 x2 t1 (bin k1 x1 t2 t3)
singleR _ _ _ _ = error "singleR"

doubleL :: a -> b -> LogMap a b -> LogMap a b -> LogMap a b
doubleL k1 x1 t1 (Bin _ k2 x2 (Bin _ k3 x3 t2 t3) t4) = bin k3 x3 (bin k1 x1 t1 t2) (bin k2 x2 t3 t4)
doubleL _ _ _ _ = error "doubleL"

doubleR :: a -> b -> LogMap a b -> LogMap a b -> LogMap a b
...
--------------------------------------------------------------------}
bin :: k -> a -> LogMap k a -> LogMap k a -> LogMap k a
...
--------------------------------------------------------------------}
instance (Eq k,Eq a) => Eq (LogMap k a) where
...
instance (Ord k, Ord v) => Ord (LogMap k v) where
    compare m1 m2 = compare (toAscList m1) (toAscList m2)

{--------------------------------------------------------------------
  Functor
--------------------------------------------------------------------}
instance Functor (LogMap k) where
  fmap f m  = map f m

instance Traversable (LogMap k) where
  traverse _ Tip = pure Tip
  traverse f (Bin s k v l r)
    = flip (Bin s k) <$> traverse f l <*> f v <*> traverse f r

instance Foldable (LogMap k) where
...
{--------------------------------------------------------------------
  Read
--------------------------------------------------------------------}
instance (Ord k, Read k, Read e) => Read (LogMap k e) where
...
{--------------------------------------------------------------------
  Show
--------------------------------------------------------------------}
instance (Show k, Show a) => Show (LogMap k a) where
  showsPrec d m  = showParen (d > 10) $
    showString "fromList " . shows (toList m)

-- | /O(n)/. Show the tree that implements the map. The tree is shown
-- in a compressed, hanging format. See 'showTreeWith'.
showTree :: (Show k,Show a) => LogMap k a -> String
showTree m
...
>  LogMap> let t = fromDistinctAscList [(x,()) | x <- [1..5]]
>  LogMap> putStrLn $ showTreeWith (\k x -> show (k,x)) True False t
>  (4,())
>  +--(2,())
>  |  +--(1,())
>  |  +--(3,())
>  +--(5,())
>
>  LogMap> putStrLn $ showTreeWith (\k x -> show (k,x)) True True t
>  (4,())
>  |
>  +--(2,())
>  |  |
...
-}
showTreeWith :: (k -> a -> String) -> Bool -> Bool -> LogMap k a -> String
showTreeWith showelem hang wide t
  | hang      = (showsTreeHang showelem wide [] t) ""
  | otherwise = (showsTree showelem wide [] [] t) ""

showsTree :: (k -> a -> String) -> Bool -> [String] -> [String] -> LogMap k a -> ShowS
...
showsTreeHang :: (k -> a -> String) -> Bool -> [String] -> LogMap k a -> ShowS
...
#include "Typeable.h"
INSTANCE_TYPEABLE2(LogMap,mapTc,"LogMap")
...
valid :: Ord k => LogMap k a -> Bool
valid t = balanced t && ordered t && validsize t

ordered :: Ord a => LogMap a b -> Bool
